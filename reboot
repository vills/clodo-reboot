...
if [ ! -f /root/.clodo-reboot ]; then
	echo "You need put your auth data to /root/.clodo-reboot"
	exit
fi

# some loooong and terrible line. will change it
SERVER_NUM=$(cat /proc/cmdline | tr " " "\n" | grep uos_id | sed -e 's|uos_id=||' | tr "-" " " | awk '{print $2}')
